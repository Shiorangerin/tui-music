...
一个用 Rust 编写的终端音乐播放器，扫描 `~/Music` 下的音频文件并以漂亮的 TUI 列表呈现，支持实时频谱可视化。
...
├── app.rs         应用状态与键盘处理
├── ui.rs          ratatui 渲染（列表 / 可视化 / 状态栏）
├── library.rs    音乐目录扫描 & 标签读取  [库]
├── player.rs      rodio 播放封装 + VizBuffer 分流 [库]
└── viz.rs         FFT 频谱计算           [库]
```

播放器用 `MonoTee` 包装解码源：解码出的多声道样本下混为 mono，一边送往 cpal 播放，一边写入环形缓冲 `VizBuffer`。可视化模块从该缓冲取出最近的 1024 个采样，加汉宁窗后做 FFT，按频段聚合为 64 条柱状能量。
